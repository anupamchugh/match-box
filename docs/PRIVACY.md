...
Machiss is a local macOS companion for reviewing visible dating-app context.
...
The capture frame is discarded after local OCR. Machiss stores approved data
...
while Machiss is closed removes the local Machiss history. This is a
developer-stage deletion path; a released app must expose a clear in-app
delete-history control before distribution.

## What it does not do

Machiss has no dating-service login, API integration, background monitoring,
